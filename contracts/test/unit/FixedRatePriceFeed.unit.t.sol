...
import {FixedRatePriceFeed} from "../../oracles/FixedRatePriceFeed.sol";
import {IFixedRatePriceFeed} from "../../interfaces/IFixedRatePriceFeed.sol";

/// @title Fixed rate price feed unit test
/// @notice U:[FRPF]: Unit tests for fixed rate price feed
contract FixedRatePriceFeedUnitTest is Test {
    FixedRatePriceFeed priceFeed;

    PriceFeedMock underlyingPriceFeed;
    ERC20Mock lpToken;

    address owner = makeAddr("owner");
    uint256 initialRate = 0.5e18;
    ...
    function setUp() public {
        ...
        priceFeed = new FixedRatePriceFeed(
            owner, initialRate, rateScale, address(underlyingPriceFeed), stalenessPeriod, address(lpToken)
        );
    }
    ...
    function test_U_FRPF_01_constructor_works_as_expected() public {
        ...
        new FixedRatePriceFeed(owner, initialRate, rateScale, address(0), stalenessPeriod, address(lpToken));

        vm.expectRevert(ZeroAddressException.selector);
        new FixedRatePriceFeed(owner, initialRate, rateScale, address(underlyingPriceFeed), stalenessPeriod, address(0));
        ...
        new FixedRatePriceFeed(
            owner, initialRate, rateScale, address(invalidPriceFeed), stalenessPeriod, address(lpToken)
        );
        ...
        assertEq(priceFeed.rate(), initialRate, "Incorrect rate");
        assertEq(priceFeed.scale(), rateScale, "Incorrect scale");
    ...
    function test_U_FRPF_02_price_feed_has_correct_metadata() public view {
        ...
        assertEq(priceFeed.description(), "LP / USD LP fixed rate price feed", "Incorrect description");
        assertTrue(priceFeed.skipPriceCheck(), "Incorrect skipPriceCheck");
        assertEq(priceFeed.contractType(), "PRICE_FEED::FIXED_RATE", "Incorrect contractType");
    ...
    function test_U_FRPF_03_latestRoundData_works_as_expected() public {
        // With initial rate 0.5e18/1e18, 1e8 price should be scaled to 0.5e8
    ...
    /// @notice U:[FRPF-4]: `setRate` works as expected
    function test_U_FRPF_04_setRate_works_as_expected() public {
        // Only owner can set rate
        vm.prank(makeAddr("notOwner"));
        vm.expectRevert("Ownable: caller is not the owner");
        priceFeed.setRate(0.75e18);

        // Cannot set rate to 0
        vm.prank(owner);
        vm.expectRevert(IFixedRatePriceFeed.RateCantBeZeroException.selector);
        priceFeed.setRate(0);

        // Set new rate and verify it works
        vm.prank(owner);
        priceFeed.setRate(0.75e18);
        assertEq(priceFeed.rate(), 0.75e18, "Rate not updated");

        // Check that the new rate affects price calculation
        underlyingPriceFeed.setPrice(1e8);
        (, int256 answer,,,) = priceFeed.latestRoundData();
        assertEq(answer, 0.75e8, "Incorrect answer with new rate");
    }
    ...
    function test_U_FRPF_05_serialize_works_as_expected() public view {
        ...
        (address _lpToken, uint256 _rate, address _priceFeed) = abi.decode(serialized, (address, uint256, address));

        assertEq(_lpToken, address(lpToken), "Incorrect serialized lpToken");
        assertEq(_rate, initialRate, "Incorrect serialized rate");
